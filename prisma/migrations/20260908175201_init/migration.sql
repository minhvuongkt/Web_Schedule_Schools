...
-- Partial unique indexes: hard timetable invariants (see prisma/schema.prisma,
-- TimetableEntry comment). Prisma cannot express `WHERE status != 'CANCELLED'`
-- in the schema, so these live in SQL only.
-- 1) A class slot holds at most one active lesson.
CREATE UNIQUE INDEX "TimetableEntry_active_class_slot_key"
  ON "TimetableEntry"("versionId", "academicDayId", "periodId", "classId")
  WHERE "status" != 'CANCELLED';
-- 2) A teacher slot holds at most one active lesson (SUBSTITUTED entries keep
--    the original teacher and still occupy the slot).
CREATE UNIQUE INDEX "TimetableEntry_active_teacher_slot_key"
  ON "TimetableEntry"("versionId", "academicDayId", "periodId", "teacherId")
  WHERE "status" != 'CANCELLED';
-- 3) A room slot holds at most one active lesson.
