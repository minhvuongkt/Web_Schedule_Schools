...
import { cancelLesson } from "@/server/services/live-ops.service";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    return NextResponse.json(await cancelLesson(parsed.data.entryId, parsed.data.reason ?? null, auth.user));
  } catch (error) {
