...
import { confirmSubstitution } from "@/server/services/live-ops.service";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    return NextResponse.json(await confirmSubstitution(parsed.data.substitutionId, auth.user));
  } catch (error) {
