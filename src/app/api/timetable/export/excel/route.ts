...
/**
 * GET /api/timetable/export/excel?scope=school|class|teacher|workload|assignments
 *   &weekId=&classId=&teacherId=
 *
 * Downloads an .xlsx built from the week's PUBLISHED version.
 * Auth: "export:excel" (TIMETABLE_ADMIN / PRINCIPAL / SUPER_ADMIN).
 */
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    const { buffer, fileName } = await exportTimetableExcel({
      scope,
      weekId,
      classId,
      teacherId,
    ...
    return errorResponse(500, "INTERNAL", "Lỗi máy chủ khi xuất tệp Excel.");
  }
}
